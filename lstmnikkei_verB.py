# -*- coding: utf-8 -*-
"""
Created on Mon Dec 18 11:08:16 2017
...
testPredict2long=list(testPredictLb)
for i in range(daylong):
     testPredict3 = []
     testPredictLb = model.predict(testPredictX)
     testPredict2long.extend(list(testPredictLb))
     testPredict3.extend(testPredict2long[len(testPredict2long)-look_back:len(testPredict2long)])
     testPredict3.extend(testPredictLb)
     hist=model.train_on_batch(testPredictX, testPredictLb)
     testPredictX2 = predict_dataset(testPredict3)
     testPredictX = testPredictX2[:,:,:,0] 

plt.plot(range(len(testX),len(testX)+look_back+daylong),testPredict2long)
plt.plot(testY,label='true')
plt.legend()
plt.title('long term predict')
plt.savefig("{}_long.png".format(daylong))
plt.show()
