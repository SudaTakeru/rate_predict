# -*- coding: utf-8 -*-
"""
Created on Tue Feb 13 16:27:51 2018
...
### 今日から先を知りたい時用
#     testPredict3 = []
#     testPredict3.extend(demoY[:-look_back])
#     testPredict3.extend(testPredict2long)
#     testPredictX = predict_dataset(testPredict3)
#testPredict2long=[]
...
#tl=2*timedelay+2*look_back+len(trainX)+len(testX)
#plt.plot(range(tl,tl+daylong),testPredict2long)
#plt.plot(range(tl,tl+daylong-1),testPredict2long2,label='upper')
#plt.plot(range(tl,tl+daylong-1),testPredict2long3,label='lower')
#plt.plot(range(tl,tl+daylong+1),dataset[tl:tl+daylong+1],label='true')
##plt.plot(range(len(trainX)+2*look_back+2*timedelay,2*timedelay+2*look_back+len(trainX)+len(testX)),testY,label='true')
##plt.plot(range(len(trainX)+len(testX)+3*look_back+3*timedelay,3*timedelay+3*look_back+len(trainX)+len(testX)+len(demoY)),demoY,label='true')
#plt.legend()
#plt.title('long term predict')
#plt.show()
#
...
#plt.plot(range(len(trainX)+len(testX)+3*look_back+3*timedelay,3*timedelay+3*look_back+len(trainX)+len(testX)+len(demoY)),demoY)
#plt.plot(range(len(trainX)+2*look_back+2*timedelay,2*timedelay+2*look_back+len(trainX)+len(testX)),testY)
#
#
#testPredict = model.predict(testX[0:look_back])
#testPredictX2 = predict_dataset(testPredict)
...
#for i in range(daylong):
#     testPredict3 = []
#     testPredict3.extend(testY[-look_back+i:])
#     testPredict3.extend(testPredict2long)
#     testPredictX = predict_dataset(testPredict3)
#     testPredictLb = model.predict(testPredictX)
#     testPredict2long.extend(list(testPredictLb))
